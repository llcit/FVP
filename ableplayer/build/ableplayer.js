...
(function ($) {
  ...
  window.AblePlayer = function(media) {
    ...
    if ($(media).attr('editCaptions') !== undefined) {
      this.captionMode = 'edit';
    }
    else {
      this.captionMode = 'view';
    }
    // start-time
  ...
  AblePlayer.prototype.recreatePlayer = function () {
    ...
    this.setupTracks().then(function() {
      ...
      thisObj.setupAltCaptions().then(function() {
        ...
        //console.log('setupTracks().then: initDescription ');
...
*/
  ...
  AblePlayer.prototype.updatePrefs = function () {
    ...
    //console.log('updatePrefs: initDescription');
...
(function ($) {
  ...
  function getCuePayload(state) {
    ...
    var result = {type: 'internal', tagName: '', value: '', classes: [], annotation: '', parent: null, children: [], language: '',category:null };
    var current = result;
    var languageStack = [];
    while (state.text.length > 0) {
      ...
      // split out coding category (wrapped in square brackets for now... )
      var catMatchPattern = /\[(\w*)\]\ /;
      var catMatches = token.value.match(catMatchPattern);
      if (catMatches) {
        current.category = catMatches[1];
        token.value = token.value.replace(catMatchPattern, "");
      }
      ...
      loadingPromise.then((function (track, kind) {
        return function (trackSrc, trackText) {
          ...
          if (kind === 'captions' || kind === 'subtitles') {
            ...
            //console.log('setupDescriptions: cues ->');
            //console.log(cues);
...
AblePlayer.prototype.showDescription = function(now) {
    ...
    var buildDescriptor = function (description,category) {
      var icons = new Map([ 
         ['professional', 'user-tie'], 
         ['cultural', 'globe'], 
         ['linguistic', 'language'] 
      ]);
      var descIcon =  icons.get(category)
      var descName = description.replace(/[^a-zA-Z0-9]/g,'_');
      var descriptionDiv = "<div id='desc_" + descName + "' name='desc_" + descName + "' class='descriptor'>" 
                           + "  <i class='fa fa-" + descIcon + " descriptor-icon descriptor-icon_"+category+"'></i>"
                           + description + "</div>";
    ...
    for (d = 0; d < cues.length; d++) {
      if ((cues[d].start <= now) && (cues[d].end > now)) {
        thisDescription = d;
        currentDescriptions.push(d);
      }
      else if (cues[d]) {

        if (cues[d].displayed) {
          console.log(cues[d]);
          console.log('displayed: ' + cues[d].displayed + ' end: ' + cues[d].end);
          console.log('now: ' + now);
        }
        if (cues[d].displayed==true && (cues[d].end < now || cues[d].start > now)) {
          var descName = cues[d].components.children[0].value.replace(/[^a-zA-Z0-9]/g,'_');
          $('#desc_'+descName).remove();
          cues[d].displayed = false;
        }
      }
    }

    for(var i=0; i<currentDescriptions.length; i++) {
      thisDescription = currentDescriptions[i];
      if (typeof thisDescription !== 'undefined') {
        if (this.currentDescription !== thisDescription && !cues[thisDescription].displayed) {
          // temporarily remove aria-live from $status in order to prevent description from being interrupted
          this.$status.removeAttr('aria-live');
          // load the new description into the container div
          cues[thisDescription].displayed = true;
          this.$descDiv.append(buildDescriptor(flattenComponentForDescription(cues[thisDescription].components),cues[thisDescription].components.category));
        ...
        this.$status.attr('aria-live','assertive');
      }
    }
  };
...
(function ($) {
  ...
  */
  ...
  AblePlayer.prototype.resizePlayer = function (width, height) {
    ...
    if (this.isFullscreen()) {
      $('.able-descriptions').appendTo(".able");
      $('.able-descriptions').css('padding-top','0%');
...
(function ($) {
  ...
  AblePlayer.prototype.generateTranscript = function(chapters, captions, descriptions) {
    ...
    if ($.inArray(this.transcriptLang,RTLLanguages) !== -1) {
    ...
    while ((nextChapter < chapters.length) || (nextDesc < descriptions.length) || (nextCap < captions.length)) {
      ...
      if (firstStart !== null) {
        if (typeof chapters[nextChapter] !== 'undefined' && chapters[nextChapter].start === firstStart) {
          ...
          if(this.captionMode == 'edit') {
            //console.log('Add caption 1 --> ',captions[nextCap]);
            addEditableCaption($main, captions[nextCap]);
          }
          else  {
...
(function ($) {
  ...
  AblePlayer.prototype.onMediaUpdateTime = function () {
    ...
    if (this.swappingSrc && (typeof this.swapTime !== 'undefined')) {
      ...
      this.updateCaption();
      //console.log('onMediaUpdateTime : showDescription -> ');
